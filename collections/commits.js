...
// 
// Fetch a commit based off owner/repo/sha.
// 
Commits.fetchCommit = function(owner, repo, sha, callback) {
  new Commits([], { owner: owner, repo: repo }).fetch({
    success: function(commits) {
      var commit = commits.findWhere({ sha: sha });
